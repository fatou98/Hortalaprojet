...
class FrontController extends Controller
{
    ...
    {
        return $this->render('HTLImmobilierBundle:Front:ajoutproprio.html.twig');
    }
    public function saveProprioAction(Request $request){
       // print_r($_POST);
            $em = $this->getDoctrine()->getManager();
            $proprio = new Proprietaire();
            $proprio->setAdresse($_POST["adresse"]);
            $proprio->setEmail($_POST["email"]);
            $proprio->setTel($_POST["tel"]);
            $proprio->setNumPiece($_POST["numeropiece"]);
            $proprio->setNomComplet($_POST["nomcomplet"]);
            $proprio->setCodeBanque($_POST["codebanque"]);
            $em->persist($proprio);
            $em->flush();
            //$info= $em->getRepository(Reservation::class)->FindBy(array('id'=>$id));
        return $this->render('HTLImmobilierBundle:Front:ajoutproprio.html.twig', array('saved'=>'succes'));
